...
def get_raw_data(save_path, retry_time=45, retry_attempts=3):
    """
    Retrieve (and resume) XML files from Arxiv and save to HDFS path

    :param save_path: HDFS Path
    :param retry_time: Time interval between two requests
    :param retry_attempts: If error, retry this many times
    """
    start = time.clock()
    ...
    if pos_end > 0 and pos_end > pos_start:
        ...
        resume = get_resume(resume_token, 1, save_path, retry_time, retry_attempts)
    while resume != "stop":
        resume = get_resume(resume, 1, save_path, retry_time, retry_attempts)


def get_resume(token, retry, save_path, retry_time, retry_attempts):
    """
    Resume retrieval from a given token
    :param token: Resume token
    :param retry: Current attempt
    :param save_path: HDFS path
    :param retry_time: Time between requests
    :param retry_attempts: If error, retry this many times
    :return: Return resume token
    """
...
def save_to_hdfs(filename, save_path):
    """
    Save list of XMLs into HDFS
...
if __name__ == '__main__':
    get_raw_data(save_path=sys.argv[1])
